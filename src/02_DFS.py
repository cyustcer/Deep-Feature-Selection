...
def optimal_s(k, optimal_c=1, n_hidden1=50, n_hidden2=10, p=p):
    ...
    for i, s in enumerate(Ss):
        ...
        model, supp, bic, [err_train, err_test] = training(k, c, s, epochs=10)
        # Store bic values
        BIC.append(bic)
...
K = 10
for k in range(K):
    X, Y, X_test, Y_test = data_load(k) # data load for two-step error calculation
    s, BIC, model, supp, report, errs = optimal_s(k, optimal_c[k]) # single datasets training
    ...
    Report += report
    Report += "    Training Accuracy: "+str(accu_train)+"\n"
    Report += "    Testing Accuray: "+str(accu_test)+"\n"
    # Results saving
    BICs.append(BIC)
    SUPPs.append(supp)
    ...
    ERR_train.append(1-accu_train)
    ERR_test.append(1-accu_test)
...
report_file = open(DIR_res+"nonlinear_report.txt", "a")
